...
void RF24::begin(void){
  ...
  write_register(STATUSS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void RF24::powerDown(void){
  dWrite(ce_pin,0); // Guarantee CE is low on powerDown
...
bool RF24::write( const void* buf, uint8_t len, const bool multicast ){
	...
	uint8_t status = write_register(STATUSS,_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT) );
...
void RF24::reUseTX(){
		write_register(STATUSS,_BV(MAX_RT) );			  //Clear max retry flag
...
bool RF24::txStandBy(){
	...
	while( ! (read_register(FIFO_STATUS) & _BV(TX_EMPTY)) ){
		if( get_status() & _BV(MAX_RT)){
			write_register(STATUSS,_BV(MAX_RT) );
			dWrite(ce_pin,0);
	...
	dWrite(ce_pin,0);		   //Set STANDBY-I mode
	return 1;
}
...
bool RF24::txStandBy(uint32_t timeout){
	...
	while( ! (read_register(FIFO_STATUS) & _BV(TX_EMPTY)) ){
		if( get_status() & _BV(MAX_RT)){
			write_register(STATUSS,_BV(MAX_RT) );
				dWrite(ce_pin,0);
				dWrite(ce_pin,1);							  //Set re-transmit

				if(millis() - start >= timeout){
					dWrite(ce_pin,0); flush_tx(); return 0;
				}
		}
		#if defined (FAILURE_HANDLING)
...
void RF24::read( void* buf, uint8_t len ){
  ...
  write_register(STATUSS,_BV(RX_DR) | _BV(MAX_RT) | _BV(TX_DS) );
...
void RF24::dWrite(uint8_t pin, uint8_t val){
  uint8_t bit = digitalPinToBitMask(pin);
  volatile uint8_t *out;
  out = (uint8_t *)portOutputRegister(digitalPinToPort(pin));
  (val)? *out |= bit : *out &=~bit;
}
